...
const publicWebhookCors = cors({ origin: '*', methods: ['GET', 'POST', 'OPTIONS'], allowedHeaders: ['Content-Type', 'Accept', 'Authorization', 'X-Requested-With'] });
app.options('/api/lead-webhooks/:publicId(wbh_[a-fA-F0-9]+)', publicWebhookCors);
...
app.use((req, res, next) => {
  if (/^\/api\/lead-webhooks\/wbh_[a-fA-F0-9]+$/.test(req.path)) {
    next();
    return;
  }
