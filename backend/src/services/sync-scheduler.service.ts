...
async function syncPipedrive(): Promise<SyncResult> {
  const { syncPipedrive: doSync } = await import('./pipedrive.service');
  const { synced, errors } = await doSync();
