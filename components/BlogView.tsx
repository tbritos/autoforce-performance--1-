...
const BlogView: React.FC = () => {
  ...
  const [hostName, setHostName] = useState('blog.autoforce.com');
  ...
  const fetchData = async () => {
    ...
    const data = await DataService.getLandingPagesGA(startDateStr, endDateStr, hostName.trim());
    setPages(data);
    setLoading(false);
  };
  ...
  }, [dateRange, customStart, customEnd, hostName]);
        ...
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-4 mb-4 bg-autoforce-darkest p-6 rounded-xl border border-autoforce-grey/20">
            <div>
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <h2 className="text-xl font-bold text-white">Blog AutoForce</h2>
                </div>
            ...
            <div className="flex flex-col sm:flex-row items-end gap-3 w-full xl:w-auto">
                ...
                <div className="flex items-center gap-2 bg-autoforce-black border border-autoforce-grey/30 rounded-lg px-2 py-1.5 w-full sm:w-52">
                    <span className="text-[10px] text-autoforce-lightGrey uppercase tracking-wider">Host</span>
                    <input
                        type="text"
                        value={hostName}
                        onChange={(e) => setHostName(e.target.value)}
                        className="bg-transparent text-xs text-white outline-none w-full"
                        placeholder="blog.autoforce.com"
                    />
                </div>
                {dateRange === 'custom' && (
