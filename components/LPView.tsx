...
// Adicionamos 'bounceRate' e 'totalClicks' nas chaves de ordenação
type SortKey = 'name' | 'views' | 'users' | 'avgEngagementTime' | 'conversions' | 'conversionRate' | 'bounceRate' | 'totalClicks';
...
const LPView: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    const data = await DataService.getLandingPagesGA(startDateStr, endDateStr);
    setPages(data);
    setLoading(false);
  };
  ...
  const sortedPages = [...filteredPages].sort((a, b) => {
      ...
      switch (sortKey) {
          ...
          case 'views': valA = a.views || 0; valB = b.views || 0; break;
          case 'users': valA = a.users || 0; valB = b.users || 0; break;
          case 'conversions': valA = a.conversions || 0; valB = b.conversions || 0; break;
          case 'conversionRate': valA = a.conversionRate || 0; valB = b.conversionRate || 0; break;
        ...
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-4 mb-4 bg-autoforce-darkest p-6 rounded-xl border border-autoforce-grey/20">
            ...
            <div className="flex flex-col sm:flex-row items-end gap-3 w-full xl:w-auto">
                 <div className="relative flex-1 w-full sm:w-64">
                    ...
                    <input type="text" placeholder="Buscar página..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} className="w-full bg-autoforce-black border border-autoforce-grey/30 text-white pl-9 pr-4 py-1.5 rounded-lg text-sm focus:border-autoforce-blue outline-none" />
                 </div>
                 <div className="flex items-center gap-2 bg-autoforce-black p-1.5 rounded-lg border border-autoforce-grey/30">
                    ...
                    <select value={dateRange} onChange={(e) => setDateRange(e.target.value)} className="bg-transparent text-white text-xs font-bold outline-none cursor-pointer p-1">
        ...
        <div className="bg-autoforce-darkest border border-autoforce-grey/20 rounded-lg overflow-hidden shadow-lg flex flex-col min-h-[400px]">
            {loading ? (
                <div className="flex-1 flex flex-col items-center justify-center text-autoforce-lightGrey gap-4">
                    ...
                    <p>Buscando dados no Google Analytics...</p>
                </div>
            ) : (
                <>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-autoforce-black/40 text-autoforce-lightGrey text-xs uppercase tracking-wider border-b border-autoforce-grey/20">
                                ...
                                <th className="p-4 text-right cursor-pointer hover:text-white" onClick={() => handleSort('users')}><div className="flex items-center justify-end">Usuários <SortIcon columnKey="users"/></div></th>
                                {/* NOVA COLUNA CLICKS */}
                                <th className="p-4 text-right cursor-pointer hover:text-white" onClick={() => handleSort('totalClicks')}><div className="flex items-center justify-end">Clicks/Eventos <SortIcon columnKey="totalClicks"/></div></th>
                                <th className="p-4 text-right cursor-pointer hover:text-white" onClick={() => handleSort('avgEngagementTime')}><div className="flex items-center justify-end">Tempo <SortIcon columnKey="avgEngagementTime"/></div></th>
                                {/* NOVA COLUNA REJEIÇÃO */}
                                <th className="p-4 text-right cursor-pointer hover:text-white" onClick={() => handleSort('bounceRate')}><div className="flex items-center justify-end">Rejeição <SortIcon columnKey="bounceRate"/></div></th>
                                <th className="p-4 text-right cursor-pointer hover:text-white" onClick={() => handleSort('conversions')}><div className="flex items-center justify-end">Conv. <SortIcon columnKey="conversions"/></div></th>
                            </tr>
                        </thead>
                            ...
                            {paginatedPages.length > 0 ? (
                                paginatedPages.map((page) => (
                                    <tr key={page.id} className="hover:bg-autoforce-blue/5 transition-colors group">
                                        ...
                                        <td className="p-4 text-right font-bold text-green-400 font-mono">{page.conversions.toLocaleString()}</td>
                                    </tr>
                                ))
                            ) : (
                                <tr><td colSpan={6} className="p-8 text-center text-autoforce-lightGrey">Nenhuma página encontrada.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                <div className="p-4 border-t border-autoforce-grey/20 flex items-center justify-between bg-autoforce-black/20 mt-auto">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-white px-2">Pág {currentPage}</span>
