...
import { MessageCircle, Search, RefreshCw, ExternalLink, CheckCheck, Send } from 'lucide-react';
...
export default function WhatsAppInboxView() {
  ...
  useEffect(() => {
    const mediaMessages = messages.map(message => {
      ...
      const embedded = ['image', 'video', 'document'].map(type => parameter?.[type]?.id).find(Boolean) ?? null;
      return { message, mediaId: message.mediaId || embedded };
    }).filter(({ message, mediaId }) => mediaId && (['image', 'audio', 'sticker', 'video', 'document'].includes(message.type) || message.type === 'template'));
    if (!mediaMessages.length) return;
    let cancelled = false;
    const urls: string[] = [];
    Promise.all(mediaMessages.map(async ({ mediaId }) => {
      try {
        const blob = await DataService.getWhatsAppMedia(String(mediaId));
        const url = URL.createObjectURL(blob);
        urls.push(url);
        return [String(mediaId), url] as const;
      } catch { return null; }
    })).then(entries => {
      if (!cancelled) setWhatsAppMedia(previous => ({ ...previous, ...Object.fromEntries(entries.filter((entry): entry is readonly [string, string] => Boolean(entry))) }));
    });
    return () => { cancelled = true; urls.forEach(url => URL.revokeObjectURL(url)); };
  }, [messages]);
  useEffect(() => { setDraft(''); }, [selected?.key]);
  const sendReply = async () => {
    if (!selected?.leadId || !draft.trim() || sending) return;
    setSending(true);
    try {
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      const container = messagesEndRef.current?.querySelector<HTMLElement>('div[style*="overflow"]');
